...
def send_report(meeting_id):
    try:
        ...
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #7c3aed, #a78bfa); padding: 24px; border-radius: 12px 12px 0 0; color: white;">
                <h1 style="margin: 0; font-size: 22px;">{report.get('title', 'Meeting Report')}</h1>
                <p style="margin: 8px 0 0; opacity: 0.9;">Generated by MeetPilotAI</p>
            </div>
            <div style="background: white; padding: 24px; border: 1px solid #e5e7eb; border-radius: 0 0 12px 12px;">
                {report.get('content_html', '<p>No content available.</p>')}
            </div>
            <p style="text-align: center; color: #9ca3af; font-size: 12px; margin-top: 16px;">
                Sent via MeetPilotAI — AI-powered meeting minutes
            </p>
        </div>
        """
